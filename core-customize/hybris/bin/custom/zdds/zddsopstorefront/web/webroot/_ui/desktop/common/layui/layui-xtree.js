...
layuiXtree.prototype.dataBind = function (d) {
    ...
    if (d.length > 0) {
        for (i in d) {
            ...
            if(d[i].checked==0){
                _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + ' value="' + d[i].uid + '" title="' + d[i].name + '" lay-skin="primary" lay-filter="xtreeck">';
            }else{
                _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + ' value="' + d[i].uid + '" title="' + d[i].name + '" lay-skin="primary" lay-filter="xtreeck" checked>';
            }
            if(d[i].child!=undefined) {
                _this.dataBind(d[i].child);
            }
            _this._domStr += '</div>';
        }
    }
...
//获取全部选中的末级checkbox对象
layuiXtree.prototype.GetParentChecked = function () {
    var arr = new Array();
    var arrIndex = 0;
    var cks = document.getElementsByClassName('layui-xtree-checkbox');
    for (var i = 0; i < cks.length; i++) {
        if (cks[i].checked && cks[i].getAttribute('data-xend') == null) {
            arr[arrIndex] = cks[i]; arrIndex++;
        }
    }
    return arr;
